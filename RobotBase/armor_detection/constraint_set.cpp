...
void ConstraintSet::LoadParam() {
	...
	//armor info
	float armor_width = 120;
	float armor_height = 60;

	//algorithm threshold parameters
        light_max_aspect_ratio_ = 20.0;
	light_min_area_ = 1;
        light_max_angle_ = 20.0;
        light_max_angle_diff_ = 8;
        armor_max_angle_ = 12.8;
	armor_min_area_ = 100;
        armor_max_aspect_ratio_ = 6;
	armor_max_pixel_val_ = 150.0;
	armor_max_stddev_ = 90;
	armor_max_mean_ = 90;
...
int ConstraintSet::DetectArmor(bool& detected, cv::Point3f& target_3d, cv::Mat& depth_img) {
	...
	while (true) {
		...
		read_index_ = cv_toolbox_->NextImage(src_img_, depth_img_);
		if (read_index_ < 0) {
...
//#if OPENCV_CUDA_ENABLE
//		cv::cuda::GpuMat src_gpu, dst_gpu;
//		src_gpu.upload(gray_img_);
//		cv::cuda::threshold(src_gpu, dst_gpu, color_thread_, 255, cv::THRESH_BINARY);
//		dst_gpu.download(binary_brightness_img);
//#else
...
//#endif
		...
		if (enemy_color_ == BLUE)
			...
			thresh = red_thread_;
//#if OPENCV_CUDA_ENABLE
//		src_gpu.upload(light);
//		cv::cuda::threshold(src_gpu, dst_gpu, thresh, 255, cv::THRESH_BINARY);
//		dst_gpu.download(binary_color_img);
//#else
		cv::threshold(light, binary_color_img, thresh, 255, cv::THRESH_BINARY);
//#endif
		if (enable_debug_ && false)
...
void ConstraintSet::PossibleArmors(const std::vector<cv::RotatedRect>& lights, std::vector<ArmorInfo>& armors) {
	...
	for (unsigned int i = 0; i < lights.size(); i++) {
		for (unsigned int j = i + 1; j < lights.size(); j++) {
			...
			/*
			if (enable_debug_) {SolveArmorCoordinate
				std::cout << "*******************************" << std::endl;
			...
			*/
			...
			if (angle_diff > 175) {
				angle_diff = 180 - angle_diff;
                        ...
                        //printf("armor_aspect_ratio_: %0.2f\n", rect.size.width / (float)(rect.size.height));
			if (angle_diff < light_max_angle_diff_ &&
                                std::max<float>(edge1.second, edge2.second) / std::min<float>(edge1.second, edge2.second) < 2.0 &&
				((rect.size.width / (rect.size.height) <4 && rect.size.width / (rect.size.height)>1.5) || (rect.size.width / (rect.size.height) <4.77 && rect.size.width / 					(rect.size.height) >4.2)) &&
...
void ConstraintSet::FilterArmors(std::vector<ArmorInfo>& armors) {
	...
	for (auto armor_iter = armors.begin(); armor_iter != armors.end();) {
		...
		if (stddev > armor_max_stddev_ || mean > armor_max_mean_ || armor_iter->rect.center.y < RE2_FRAME_HEIGHT * 0.2) {
...
ArmorInfo ConstraintSet::SlectFinalArmor(std::vector<ArmorInfo>& armors) {
	//这里有两种方案
	//DJI的是选择最大的可能装甲,我的是识别角度最小的可能装甲
	//两种都无法避免对方车露出两个装甲时的云台摆动问题
        //std::sort(armors.begin(),
        //	armors.end(),
        //        [](const ArmorInfo& p1, const ArmorInfo& p2) { return p1.rect.size.area() > p2.rect.size.area(); });
        int min_index = 0;
        int min_dis = 0;
        int dx = armors[0].rect.center.x- (RE2_FRAME_WIDTH/2);
        int dy = armors[0].rect.center.y - (RE2_FRAME_HEIGHT/2);
        min_dis = std::sqrt(dx * dx + dy * dy);        \
        for (unsigned int i  = 1; i < armors.size(); i++)
        {
            dx = armors[i].rect.center.x- (RE2_FRAME_WIDTH/2);
            dy = armors[i].rect.center.y - (RE2_FRAME_HEIGHT/2);
            int dis = std::sqrt(dx * dx + dy * dy);
            if (dis < min_dis)
                min_index = i;

        }
        return armors[min_index];
}

void ConstraintSet::CalcControlInfo(const ArmorInfo& armor, cv::Point3f& target_3d)
{
	target_3d.x = armor.rect.center.x;
	target_3d.y = armor.rect.center.y;
}
